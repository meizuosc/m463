...
static int lsm330_gyr_get_raw_data(struct lsm330_gyr_status *stat,
...
static int selftest_init(struct lsm330_gyr_status *gyr)
{
	unsigned char buf[6];
	//	pr_info("%s\n", __func__);
	...
	return lsm330_gyr_i2c_write(gyr, &buf[0], 5);
}

static int selftest_enable(struct lsm330_gyr_status *gyr)
{
	/*
	...
	return lsm330_gyr_i2c_write(gyr, buf, 1);
}


static void selftest_disable(struct lsm330_gyr_status *gyr)
{
	u8 buf[2];
	buf[1]= 0x00;
	//	pr_info("%s\n", __func__);
	...
	/* Disable sensor */
	buf[0] = CTRL_REG1;
	lsm330_gyr_i2c_write(gyr, buf, 1);
	/* Disable selftest */
	buf[0] = CTRL_REG4;
	lsm330_gyr_i2c_write(gyr, buf, 1);

	if (atomic_read(&gyr->enabled)) {
		lsm330_gyr_device_power_on(gyr);
	}
}

static int selftest_wait_ZYXDA(struct lsm330_gyr_status *gyr)
{
	int i, ret;
	...
	for (i = SELFTEST_MEASURE_TIMEOUT; i != 0; i--) {
		...
		ret = lsm330_gyr_i2c_read(gyr, &data_ready, 1);
		if (ret < 0) {
			pr_err("%s: lsm330_gyr_i2c_read fail, retry %d\n", __func__, i);
...
static int selftest_read(struct lsm330_gyr_status *stat,
		struct lsm330_gyr_triple *data)
{
	int total[3];
	int i, ret;
	...
	for (i = 0; i < SELFTEST_SAMPLES; i++) {
		...
		ret = lsm330_gyr_get_raw_data(stat,data);
		if (ret < 0) {
...
static inline int selftest_check(struct lsm330_gyr_triple *data_nost, struct lsm330_gyr_triple *data_st)
{
	pr_debug("%s\n", __func__);
...
static int lsm330_gyr_selftest_result(struct lsm330_gyr_status *stat,int *test_result)
{
	int ret;
	struct lsm330_gyr_triple data_nost, data_st;
...
#if 1
	for (i=1; i>0; i--) {
		ret = lsm330_gyr_selftest_result(stat, &self_test);
		if (ret < 0 || self_test==0) {
			dev_err(dev, "failed to perform gyr self test\n");
			break;
		}
	}
#else
...
static void lsm330_gyr_input_work_func(struct work_struct *work)
{
	...
	stat = container_of((struct delayed_work *)work,
		struct lsm330_gyr_status, input_work);
...
static struct i2c_board_info i2c_lsm330_gyr[] = {
	{
...
static int __init lsm330_gyr_init(void)
{
	...
	i2c_register_board_info(1, i2c_lsm330_gyr, 1);
