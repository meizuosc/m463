...
static struct iio_dev *inv_iio_dev_g = NULL;
		...
		u8 reg, u16 size, u8 *data)
{
	...
	ret = mt_i2c_master_recv(st->client, (const char*)phyAddr, len, I2C_DMA_FLAG);
	if (ret < 0) {
	...
	struct device_attribute *attr, char *buf)
{
	...
	case ATTR_SECONDARY_NAME:
	{
		const char *n[] = {"NULL", "AK8975", "AK8972", "AK8963",
					"BMA250", "MLX90399", "AK09911", "ST480M"};
...
static ssize_t inv_sensor_phone_calling_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct inv_mpu_state *st = iio_priv(dev_get_drvdata(dev));
	int data;

	kstrtoint(buf, 10, &data);
	st->phone_calling_state = !!data;

	return count;
}

static ssize_t inv_sensor_phone_calling_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct inv_mpu_state *st = iio_priv(dev_get_drvdata(dev));

	return sprintf(buf, "%d\n", st->phone_calling_state);
}
...
static DEVICE_ATTR(sensor_phone_calling, S_IRUGO | S_IWUSR,
		inv_sensor_phone_calling_show,
		inv_sensor_phone_calling_store);
...
{
	struct iio_dev *indio_dev = inv_iio_dev_g;
	struct inv_mpu_state *st = iio_priv(indio_dev);
	int result;

	/* add code according to different request Start */
	pr_info("%s inv_early_suspend\n", st->hw->name);
	if(!st->phone_calling_state)
		mt_eint_mask(st->irq);
}
...
msleep(100);
	...
	st->phone_calling_state = 0;
#ifdef CONFIG_HAS_EARLYSUSPEND
	st->inv_suspend.suspend = inv_early_suspend;
	st->inv_suspend.resume = inv_late_resume;
	st->inv_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1;
	register_early_suspend(&st->inv_suspend);
#endif
...
static struct mpu_platform_data gyro_platform_data = {
	...
	//.sec_slave_id   = COMPASS_ID_ST480M,
	...
	.secondary_orientation = { -1,  0, 0,
		...
		0,  0, 1 },
		*/
	//ak09911c orientation
	.secondary_orientation = { 0,  1, 0,
		1,  0,  0,
		0,  0, -1 },
	.key = {0x9c, 0xc7, 0x49, 0x7, 0xb, 0x3, 0x34, 0x74, 0x9d,
			0x65, 0x52, 0x99, 0xa8, 0xac, 0xc5, 0x5b},
};
...
{
	pr_info("INV MPU6515A probe start\n");
