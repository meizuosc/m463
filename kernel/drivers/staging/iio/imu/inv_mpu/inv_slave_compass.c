...
#define DATA_ST480M_SCALE           (4915 * (1L << 15))
...
#define ST480M_REG_DRR_SHIFT	2
#define ST480M_DEVICE_ID	0x7c
#define ST480M_I2C_RETRY_NUM	3
#define ST480M_DATA_BYTES	7
#define DATA_ST480M_MIN_READ_TIME            (27 * NSEC_PER_MSEC)

/*
 * st480m register
 */
#define ST480M_SINGLE_MEASUREMENT_MODE_CMD	0x3E
#define ST480M_READ_MEASUREMENT_CMD	0x4E
#define ST480M_WRITE_REGISTER_CMD	0x60
#define ST480M_READ_REGISTER_CMD	0x50
#define ST480M_EXIT_REGISTER_CMD	0x80
#define ST480M_MEMORY_RECALL_CMD	0xD0
#define ST480M_MEMORY_STORE_CMD		0xE0
#define ST480M_RESET_CMD		0xF0

#define ST480M_CALIBRATION_REG (0x02 << ST480M_REG_DRR_SHIFT)
#define ST480M_CALIBRATION_DATA_LOW 0x1C
#define ST480M_CALIBRATION_DATA_HIGH 0x00

#define ST480M_ONE_INIT_DATA_LOW 0x7C
#define ST480M_ONE_INIT_DATA_HIGH 0x00
#define ST480M_ONE_INIT_REG (0x00 << ST480M_REG_DRR_SHIFT)

#define ST480M_TWO_INIT_DATA_LOW 0x00
#define ST480M_TWO_INIT_DATA_HIGH 0x00
#define ST480M_TWO_INIT_REG (0x02 << ST480M_REG_DRR_SHIFT)
...
static const short AK09911_ST_Lower[3] = {-50, -50, -400};
static const short AK09911_ST_Upper[3] = {50, 50, -100};
...
static int inv_check_akm_self_test(struct inv_mpu_state *st)
{
	...
	while (counter > 0) {
		...
		if (COMPASS_ID_AK09911 == st->plat_data.sec_slave_id) {
			result = inv_secondary_read(REG_AK09911_STATUS1,
					1, data);
		} else
 ...
 * read/write
 */
static int st480m_i2c_transfer_data(struct inv_mpu_state *st,
		int len, char *buf, int length)
{
	...
	int count = 0;
	while (i2c_transfer(st->client->adapter, msgs, 2) < 0) {
		pr_warn("st480m i2c transfer error, retry: %d\n", count++);
		if(count > ST480M_I2C_RETRY_NUM - 1)
			return -EIO;
	}
	return 0;
}
...
/*
 *  inv_setup_compass_senodia() - Configure senodia series compass.
 */
static int inv_setup_compass_senodia(struct inv_mpu_state *st)
{
	int result, i;
	u8 data[7];
	u8 sens, mode, cmd;
	...
	if (result)
		return result;
	/* read secondary i2c ID register */
	data[0] = ST480M_READ_REGISTER_CMD;
	data[1] = 0x00;
	result = st480m_i2c_transfer_data(st, 2, data, 3);
	printk("st480 device id: %x\n", data[2]);
	if (result) {
		pr_err("st480 read device id error\n");
		return result;
	}
	if (data[2] != ST480M_DEVICE_ID) {
		pr_err("invalid device id: %x, aborted\n", data[2]);
		return -ENXIO;
	}

	data[0] = ST480M_WRITE_REGISTER_CMD;
	data[1] = ST480M_ONE_INIT_DATA_HIGH;
	data[2] = ST480M_ONE_INIT_DATA_LOW;
	data[3] = ST480M_ONE_INIT_REG;
	result = st480m_i2c_transfer_data(st, 4, data, 1);
	if(result) {
		pr_err("st480 write init_1 reg error\n");
		return result;
	}

	data[0] = ST480M_WRITE_REGISTER_CMD;
	data[1] = ST480M_TWO_INIT_DATA_HIGH;
	data[2] = ST480M_TWO_INIT_DATA_LOW;
	data[3] = ST480M_TWO_INIT_REG;
	result = st480m_i2c_transfer_data(st, 4, data, 1);
	if(result) {
		pr_err("st480 write init_2 reg error\n");
		return result;
	}

	data[0] = ST480M_WRITE_REGISTER_CMD;
	data[1] = ST480M_CALIBRATION_DATA_HIGH;
	data[2] = ST480M_CALIBRATION_DATA_LOW;
	data[3] = ST480M_CALIBRATION_REG;
	result = st480m_i2c_transfer_data(st, 4, data, 1);
	if(result) {
		pr_err("st480 write calibration reg error\n");
		return result;
	}

	data[0] = ST480M_SINGLE_MEASUREMENT_MODE_CMD;
	result = st480m_i2c_transfer_data(st, 1, data, 1);
	if(result) {
		pr_err("st480 switch mode failed\n");
		return result;
	}
	...
	msleep(30);
	memset(data, 0, 7);
	data[0] = ST480M_READ_MEASUREMENT_CMD;
	result = st480m_i2c_transfer_data(st, 1, data, 7);
	for(i = 0; i < 3; i++)
	{
		printk("st480m data status: %x, ret: %d\n", data[0], result);
		printk("st480m raw data: %d\n",
				(data[i * 2 + 1] << 8) | data[i * 2 + 2]);
	}
	*/

	st->slave_compass->scale = DATA_ST480M_SCALE;
	/* restore to non-bypass mode */
	result = inv_i2c_single_write(st, REG_INT_PIN_CFG,
			st->plat_data.int_config);
	if (result)
	...
	if (result)
		return result;
	/* slave 0 is used for write st480mc reading cmd */
	result = inv_i2c_single_write(st, REG_I2C_SLV0_ADDR,
		st->plat_data.secondary_i2c_addr);
	if (result)
		return result;
	result = inv_i2c_single_write(st, INV_MPU_REG_I2C_SLV0_DO,
			ST480M_READ_MEASUREMENT_CMD);
	if (result)
		return result;

	/* slave 1 is used for ST480M data reading */
	result = inv_i2c_single_write(st, REG_I2C_SLV1_ADDR,
			INV_MPU_BIT_I2C_READ |
			st->plat_data.secondary_i2c_addr);
	if (result)
	...
	/* slave 2 is used for ST480M mode change only */
	result = inv_i2c_single_write(st, REG_I2C_SLV2_ADDR,
		st->plat_data.secondary_i2c_addr);
	if (result)
		return result;
	/* single measure mode, ignore register address */
	result = inv_i2c_single_write(st, INV_MPU_REG_I2C_SLV2_DO,
			ST480M_SINGLE_MEASUREMENT_MODE_CMD);
	if (result)
		return result;

	/* slave 3 is used reading mode status */
	result = inv_i2c_single_write(st, REG_I2C_SLV3_ADDR,
		INV_MPU_BIT_I2C_READ | st->plat_data.secondary_i2c_addr);
	if (result)
		return result;
...
static int inv_senodia_read_data(struct inv_mpu_state *st, short *o)
{
	int result, shift;
	int i;
	u8 d[ST480M_DATA_BYTES];

	result = 0;
	memset(d, 0, ST480M_DATA_BYTES);
	if (st->chip_config.dmp_on) {
		for (i = 0; i < 6; i++)
			d[1 + i] = st->fifo_data[i];
	} else {
	...
	if (((d[0] >> 4) & 0x01) || result) {
		pr_err("st480m read data error, status: %x, ret: %d\n",
				d[0], result);
		result = -EINVAL;
	} else {
		for(i = 0; i < 3; i++)
		{
			o[i] = (d[i * 2 + 1] << 8) | d[i * 2 + 2];
		}
		o[2] = (int)(o[2] * 26) / 15;
	}
	return result;
...
static int inv_suspend_senodia(struct inv_mpu_state *st)
{
	int result;

	printk("st480m enter suspend\n");
	/* slave 0 is disabled */
	result = inv_i2c_single_write(st, REG_I2C_SLV0_CTRL, 0);
	if (result)
		return result;
...
static int inv_resume_senodia(struct inv_mpu_state *st)
{
	int result;
	u8 reg_addr, bytes;

	printk("st480m enter resume\n");
	/* write measure cmd by slave 0*/
	result = inv_i2c_single_write(st, REG_I2C_SLV0_CTRL,
						INV_MPU_BIT_REG_DIS |
						INV_MPU_BIT_SLV_EN | 1);
	if (result)
		return result;
	/* reading data by slave 1 */
	result = inv_i2c_single_write(st, REG_I2C_SLV1_CTRL,
						INV_MPU_BIT_REG_DIS |
						INV_MPU_BIT_SLV_EN |
						ST480M_DATA_BYTES);
	...
	/* write single measurement mode by slave 2*/
...
static int inv_write_senodia_scale(struct inv_mpu_state *st, int data)
{
	st->slave_compass->scale = data;
	return 0;
}

static int inv_read_senodia_scale(struct inv_mpu_state *st, int *scale)
{
	*scale = st->slave_compass->scale;
	return IIO_VAL_INT;
}

static int inv_check_senodia_self_test(struct inv_mpu_state *st)
{
	return 0;
}
...
static struct inv_mpu_slave slave_st480m = {
	.suspend   = inv_suspend_senodia,
	.resume    = inv_resume_senodia,
	.get_scale = inv_read_senodia_scale,
	.set_scale = inv_write_senodia_scale,
	.self_test = inv_check_senodia_self_test,
	.setup     = inv_setup_compass_senodia,
	.read_data = inv_senodia_read_data,
	.rate_scale = ST480M_RATE_SCALE,
	.min_read_time = DATA_ST480M_MIN_READ_TIME,
};
...
int inv_mpu_setup_compass_slave(struct inv_mpu_state *st)
{
	...
	case COMPASS_ID_MLX90399:
		...
		break;
	case COMPASS_ID_ST480M:
		st->slave_compass = &slave_st480m;
		break;
	default:
		return -EINVAL;
	}
