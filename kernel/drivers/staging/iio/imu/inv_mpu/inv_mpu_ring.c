...
static struct inv_mpu_state *g_st = NULL;
...
static int inv_send_compass_data(struct inv_mpu_state *st)
{
	...
	pr_debug("[mpu6515] compass data: %d %d %d\n", sen[0], sen[1], sen[2]);
					...
					u8 *data, s64 t)
{
	...
	pr_debug("[mpu6515] accel data %d %d %d\n",s[0], s[1], s[2]);
	if (st->sensor[SENSOR_GYRO].on) {
		for (i = 0; i < 3; i++)
			s[i] = be16_to_cpup((__be16 *)(&data[ind + i * 2]));
		inv_push_8bytes_buffer(st, GYRO_HDR, t, s);
	...
	pr_debug("[mpu6515] gyro data %d %d %d\n",s[0], s[1], s[2]);
...
void inv_read_fifo(struct work_struct *work)
{
	struct inv_mpu_state *st = container_of(work, struct inv_mpu_state,
			fifo_work);
	struct iio_dev *indio_dev = iio_priv_to_dev(st);
	int result, bpm;
...
int inv_mpu_configure_ring(struct iio_dev *indio_dev)
{
	...
	*/
	mt_eint_set_sens(st->client->irq, MT_EDGE_SENSITIVE);
	mt_eint_registration(st->client->irq, EINTF_TRIGGER_RISING,
			inv_irq_handler, true);
	mt_eint_unmask(st->client->irq);
