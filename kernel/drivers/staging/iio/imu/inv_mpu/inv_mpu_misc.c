...
int mpu_memory_write_addr(struct inv_mpu_state *st, u8 mpu_addr, u16 mem_addr)
{
	int ret =0;
	u8 bank[2],addr[2];
	...
	st->i2c_addr = mpu_addr;
	ret = i2c_master_send(st->client, &bank, sizeof(bank));
	if (ret < 0) {
		pr_err("%s send  REG_MEM_RW fail\n", __func__);
		return ret;
	}

	ret = i2c_master_send(st->client, &addr, sizeof(addr));
	if (ret < 0) {
		pr_err("%s send  REG_MEM_RW fail\n", __func__);
		return ret;
	}
...
//	pr_err("%s argument size = %d\n", __func__, size);
	...
	if(len > 254){
		int first_count = len/254*254;
		int second_count = len %254;
		ret = mpu_memory_write_addr(st, mpu_addr, mem_addr);
		ret = mt_i2c_master_send(st->client, ( char*)phyAddr, first_count|254, I2C_DMA_FLAG);
#if 0
		for(i = 0; i < first_count; i++){
			pr_err("after write 0x%2x ", buf[i+1]);
		}
#endif

		ret = mpu_memory_read(st, mpu_addr, mem_addr, first_count, buf+1);
#if 0
		for(i = 0; i < first_count; i++){
			pr_err("after read 0x%2x ", buf[i+1]);
		}
#endif
		if(second_count > 0){
			ret = mpu_memory_write_addr(st, mpu_addr, mem_addr+first_count-1);
			memcpy(buf+1,buf+first_count, second_count);
			ret = mt_i2c_master_send(st->client, ( char*)phyAddr, second_count+1, I2C_DMA_FLAG);
#if 0
			for(i = 0; i < second_count; i++){
				pr_err("after write 0x%2x ", buf[i+1]);
			}

			ret = mpu_memory_read(st, mpu_addr, mem_addr+first_count -1, second_count, buf+1);
			for(i = 0; i < second_count; i++){
				pr_err("after read 0x%2x ", buf[i+1]);
			}
#endif
		...
		ret = mpu_memory_write_addr(st, mpu_addr, mem_addr);
		ret = mt_i2c_master_send(st->client, ( char*)phyAddr, len, I2C_DMA_FLAG);
	}
...
int mpu_memory_read_addr(struct inv_mpu_state *st, u8 mpu_addr, u16 mem_addr)
{
	int ret =0;
	u8 bank[2],addr[2];
	u8 buf = REG_MEM_RW;
	...
	ret = i2c_master_send(st->client, &buf, 1);
	if (ret < 0) {
		pr_err("%s send  REG_MEM_RW fail\n", __func__);
		return ret;
	}
		...
		u32 size, u8 *data)
{
	...
	buf = dma_alloc_coherent(NULL, size, &phyAddr, GFP_KERNEL);
	if (!buf) {
		pr_err("%s(), can not allocate dma addr\n", __func__);
		return -ENOMEM;
	}
	...
	if(size > 254){
		int first_count = size /254*254;
		int second_count = size %254;
		ret |= mpu_memory_read_addr(st, mpu_addr, mem_addr);
		ret |= mt_i2c_master_recv(st->client, ( char*)phyAddr, first_count|254, I2C_DMA_FLAG);
		if(second_count > 0){
			ret |= mpu_memory_read_addr(st, mpu_addr, mem_addr+first_count);
			ret |= mt_i2c_master_recv(st->client, ( char*)phyAddr+first_count, second_count, I2C_DMA_FLAG);
		}
	}else{
		ret |= mpu_memory_read_addr(st, mpu_addr, mem_addr);
	...
	dma_free_coherent(NULL, size, buf, phyAddr);
	return (ret < 0) ? ret : 0;
}
*/
...
int inv_get_silicon_rev_mpu6500(struct inv_mpu_state *st)
{
	...
	pr_debug("[mpu6515] read REG_WHOAMI result: %d, vaule: %d\n", result,
			whoami);
	if (result)
		return result;
	...
	pr_debug("[mpu6515] read MEM_REV_ADDR result: %d, vaule: %d\n", result,
			sw_rev);
	...
	char *buf, loff_t pos, size_t size)
{
	...
	if (result) {
		pr_err("inv_load_fimware is failed\n");
		goto firmware_write_fail;
	}

	result = inv_verify_firmware(st, firmware, size);
	if (result) {
		pr_err("inv_verify_firmware failed\n");
		goto firmware_write_fail;
	}
	...
	if (result) {
		pr_err("inv_gyro_dmp_cal failed\n");
		goto firmware_write_fail;
	}
	result = inv_accel_dmp_cal(st);
	if (result) {
		pr_err("inv_accel_dmp_cal failed\n");
		goto firmware_write_fail;
	}
	result = inv_dry_run_dmp(st);
	if (result) {
		pr_err("inv_dry_run_dmp failed\n");
		goto firmware_write_fail;
	}
