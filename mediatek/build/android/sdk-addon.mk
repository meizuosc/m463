...
SDK_PLUSOPEN_NAME := mediatek_android_sdk
...
SDK_PLUSOPEN_last_version := $(lastword $(call numerically_sort, \
            $(filter-out current, \
                $(patsubst mediatek/frameworks/banyan/notes/release-%.txt,%, $(wildcard mediatek/frameworks/banyan/notes/release-*.txt)) \
             )\
        ))
...
SDK_PLUSOPEN_toolset_dir_leaf := $(SDK_PLUSOPEN_NAME)_tools-$(strip $(last_released_sdk_version)).$(strip $(last_released_mtk_sdk_version)).$(strip $(MTK_SDK_TOOLSET_VERSION))
...
SDK_PLUSOPEN_note_full_target := $(HOST_OUT_SDK_ADDON)/release-$(SDK_PLUSOPEN_last_version).txt
$(SDK_PLUSOPEN_note_full_target): mediatek/frameworks/banyan/notes/release-$(SDK_PLUSOPEN_last_version).txt
	$(hide) mkdir -p $(dir $@)
	$(hide) cp -f $< $@

.PHONY: banyan_opensdk
banyan_opensdk: $(SDK_PLUSOPEN_full_target) $(SDK_PLUSOPEN_toolset_full_target) $(SDK_PLUSOPEN_note_full_target)
