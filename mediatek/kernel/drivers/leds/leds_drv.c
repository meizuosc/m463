...
{
       //2014-10-4 add by Yin ShunQing, for led breath mode blink
#ifdef MEIZU_M71
       struct mt65xx_led_data *led_data =
               container_of(led_cdev, struct mt65xx_led_data, cdev);
       ...
       if (led_data->cust.data == MT65XX_LED_PMIC_NLED_ISINK0)
       {
                ...
                * In order to enable breath mode blink when "echo timer > /sys/class/leds/button-backlight/trigger"
                */
...
static ssize_t store_oneshot_blink(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0)
	{
		...
		LEDS_DRV_DEBUG("store_oneshot_blink: enable:%u, size:%d, address:0x%s\n", enable_oneshot, size, buf);

		if (enable_oneshot)
		{
			//mz_led_oneshot_blink(led_cdev,led_cdev->brightness);
