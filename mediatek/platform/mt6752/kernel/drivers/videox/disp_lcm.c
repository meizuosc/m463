...
disp_lcm_handle* disp_lcm_probe(char* plcm_name, LCM_INTERFACE_ID lcm_id)
{
	...
	int ret = 0;
    int lcmindex = 0;//lichao@20141112 patch mtk ALPS01808354 add
	...
	else if(_lcm_count() == 1)
	{
        ...
        lcmindex = 0;//lichao@20141112 patch mtk ALPS01808354 add
	}
	else
	{
		if(plcm_name == NULL)
		...
		else
		{
			...
			for(i=0;i<_lcm_count();i++)
			{
				...
				if(!strcmp(lcm_drv->name, plcm_name))
				{
					isLCMFound = true;
					isLCMInited = true;
                    lcmindex = i;//lichao@20141112 patch mtk ALPS01808354 add
					break;
				}
			}
//LiChao @2014.10.12 correct log info begin
			#ifdef MEIZU_M71
			if(!isLCMFound && !isLCMInited)
			#endif
//LiChao @2014.10.12 correct log info end				
	...
	if(plcm && lcm_param)
	{
        ...
        plcm->index = lcmindex;//lichao@20141112 patch mtk ALPS01808354 add
	}
	else
	{
