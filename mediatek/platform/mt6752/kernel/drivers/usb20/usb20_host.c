...
extern GPIO_OTG_DRVVBUS_PIN_NEW;
#endif
...
{
#ifdef MEIZU_M71
    unsigned int  GPIO_OTG_DRVVBUS_PIN_NEW;
    if( mz_get_hw_version() == 1 || mz_get_hw_version() == 0 )
    {
        GPIO_OTG_DRVVBUS_PIN_NEW = GPIO143;
    ...
    if(is_on){
        ...
        mt_set_gpio_out(GPIO_OTG_DRVVBUS_PIN,GPIO_OUT_ONE);
				...
				bq24296_set_en_boost(1);
				mt_set_gpio_mode(GPIO_OTG_DRVVBUS_PIN_NEW,GPIO_OTG_DRVVBUS_PIN_M_GPIO);
...
void mt_usb_init_drvvbus(void)
{
	...
	#ifdef MTK_BQ24296_SUPPORT
		mt_set_gpio_mode(GPIO_OTG_DRVVBUS_PIN_NEW,GPIO_OTG_DRVVBUS_PIN_M_GPIO);//should set GPIO2 as gpio mode.
		mt_set_gpio_dir(GPIO_OTG_DRVVBUS_PIN_NEW,GPIO_DIR_OUT);
		mt_get_gpio_pull_enable(GPIO_OTG_DRVVBUS_PIN_NEW);
		mt_set_gpio_pull_select(GPIO_OTG_DRVVBUS_PIN_NEW,GPIO_PULL_UP);
	#else
