...
#define MTK_BQ24296_GPIO
//#define GPIO_CHR_PSEL_PIN_NEW		GPIO69
//#define GPIO_SWCHARGER_EN_PIN	GPIO68
//#define GPIO_OTG_DRVVBUS_PIN_NEW	GPIO143
extern unsigned int mz_get_hw_version(void);

    kal_uint32 GPIO_CHR_PSEL_PIN_NEW;
    kal_uint32 GPIO_OTG_DRVVBUS_PIN_NEW;
...
#ifdef MTK_BQ24296_GPIO
    if( mz_get_hw_version() == 1 || mz_get_hw_version() == 0 )
     {
        GPIO_CHR_PSEL_PIN_NEW = (GPIO69 | 0x80000000);
        GPIO_OTG_DRVVBUS_PIN_NEW = (GPIO143 | 0x80000000);
     }
     else
     {
        GPIO_CHR_PSEL_PIN_NEW = GPIO15;
        GPIO_OTG_DRVVBUS_PIN_NEW = GPIO128;

     }


    //pull PSEL low.(PSEL is LOW means that the input source is Adapter, HIGH is USB Host)
    mt_set_gpio_mode(GPIO_CHR_PSEL_PIN_NEW,GPIO_MODE_GPIO);
    mt_set_gpio_dir(GPIO_CHR_PSEL_PIN_NEW,GPIO_DIR_OUT);
 //LOW means the input source is Adapter, the max input current is 3A
    mt_set_gpio_out(GPIO_CHR_PSEL_PIN_NEW,GPIO_OUT_ONE);
    ...
    {
        //printk(KERN_EMERG "+++++++++++++++++++g_boo_mode = meta +++++++++++++++++++++++++\n");
        mt_set_gpio_out(GPIO_SWCHARGER_EN_PIN,GPIO_OUT_ZERO);
        bq24296_set_chg_config(0x0);
    }
    else
    {
        //printk(KERN_EMERG "+++++++++++++++++++g_boo_mode = normal +++++++++++++++++++++++++\n");
        mt_set_gpio_out(GPIO_SWCHARGER_EN_PIN,GPIO_OUT_ZERO);
    }
    //pull OTG low.(CR LOW meas the charger enable, High is Disable)
    mt_set_gpio_mode(GPIO_OTG_DRVVBUS_PIN_NEW,GPIO_MODE_GPIO);
    mt_set_gpio_dir(GPIO_OTG_DRVVBUS_PIN_NEW,GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO_OTG_DRVVBUS_PIN_NEW,GPIO_OUT_ONE);
#endif
    ...
    bq24296_set_watchdog(0x0); //Close WDT
...
void  tbl_charger_otg_vbus(kal_uint32 mode)
{
	...
	if(state){
		...
		bq24296_set_chg_config(0x0);
#endif
		mt_set_gpio_out(GPIO_OTG_DRVVBUS_PIN_NEW, GPIO_OUT_ZERO);
	}
}
EXPORT_SYMBOL(tbl_charger_otg_vbus);
 ...
 static kal_uint32 charging_enable(void *data)
 {
	...
	if(KAL_TRUE == enable)
	{
        if( !(g_boot_mode ==  META_BOOT || g_boot_mode == ADVMETA_BOOT)  )
        {
            //printk(KERN_EMERG "+++++2+++++++++++++g_boo_mode = meta +++++++++++++++++++++++++\n");
		    bq24296_set_en_hiz(0x0);
 ...
 static kal_uint32 charging_get_battery_status(void *data)
 {
	   kal_uint32 status = STATUS_OK;
 	   ...
 	   mt6325_upmu_set_rg_baton_tdet_en(1);
	   mt6325_upmu_set_rg_baton_en(1);
	   *(kal_bool*)(data) = mt6325_upmu_get_rg_int_status_vbaton_undet();
...
#else
	 charger_type = bq24296_get_charger_type();
