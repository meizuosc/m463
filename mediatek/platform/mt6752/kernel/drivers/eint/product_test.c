...
static int enable_sim_gpio(const char *val, const struct kernel_param *kp)
{
	...
	if (sim_card_detect) {
		mt_set_gpio_mode(GPIO140 | 0x80000000, GPIO_MODE_00);
		mt_set_gpio_dir(GPIO140 | 0x80000000, GPIO_DIR_IN);
		temp = mt_get_gpio_in(GPIO140 | 0x80000000);
		printk("product test: meizu sim_card_detect enable\n");
	} else {
		mt_set_gpio_mode(GPIO140 | 0x80000000, GPIO_MODE_01);
		mt_set_gpio_dir(GPIO140 | 0x80000000, GPIO_DIR_IN);
