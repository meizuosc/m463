...
#ifdef MEIZU_M79
static GPIO_MODE mtk_uart0_pin_mode(void)
{
    return GPIO_MODE_02;
}
#else
...
{
	GPIO_MODE mode;
	unsigned int hw_ver = mz_get_hw_version();

	switch (hw_ver) {
	/* the second hw ver had switched the tx and rx pin */
	case 4:
		mode = GPIO_MODE_02;
		break;
...
void mtk_uart_switch_to_tx(struct mtk_uart *uart)
{
	...
	case 0:
	    ...
	    #else
	    mt_set_gpio_mode(GPIO_UART_UTXD0_PIN, mtk_uart0_pin_mode());
	    #endif /* end of #ifndef MEIZU_M71 */
	    #else
...
void mtk_uart_switch_to_rx(struct mtk_uart *uart)
{
	...
	case 0:
	    ...
	    #else
	    mt_set_gpio_mode(GPIO_UART_URXD0_PIN, mtk_uart0_pin_mode());
	    #endif /* end of #ifndef MEIZU_M71 */
	    #else
