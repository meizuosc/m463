...
static int amutt_proc_register(void)
{
    ...
    amutt_proc_dir = proc_mkdir("driver/thermal-cl-amutt", NULL);
    if (!amutt_proc_dir) {
        mtk_cooler_amutt_dprintk("[%s] mkdir /proc/driver/thermal-cl-amutt failed\n", __func__);
    } else {
#if 0
        entry = create_proc_entry("amutt_param", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, amutt_proc_dir);
        if (entry) {
            entry->read_proc = amutt_param_read;
            entry->write_proc = amutt_param_write;
            entry->gid = 1000; // allow system process to write this proc
        }
#endif
        entry = proc_create("param", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, amutt_proc_dir, &amutt_param_fops);
        if (entry) {
...
#if 0
        entry = create_proc_entry("amutt_asparam", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, amutt_proc_dir);
        if (entry) {
            entry->read_proc = amutt_asparam_read;
            entry->write_proc = amutt_asparam_write;
            entry->gid = 1000; // allow system process to write this proc
        }
#endif
        entry = proc_create("asparam", S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP, amutt_proc_dir, &amutt_asparam_fops);
        if (entry) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
            proc_set_user(entry, 0, 1000);
#else
...
#if 0
        entry = create_proc_entry("amutt_dbg", S_IRUSR | S_IWUSR, amutt_proc_dir);
        if (entry) {
            entry->read_proc = amutt_dbg_read;
            entry->write_proc = amutt_dbg_write;
        }
#endif
        entry = proc_create("dbg", S_IRUSR | S_IWUSR, amutt_proc_dir, &amutt_dbg_fops);
        if (entry) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
            proc_set_user(entry, 0, 1000);
#else
