...
static unsigned int _calc_new_opp_idx(struct mt_cpu_dvfs *p, int new_opp_idx)
{
    ...
    if (p->dvfs_disable_by_early_suspend) {
        ...
        #ifndef MEIZU_M71		//close too much log, by yubin, 2014.10.10
    ...
    if (likely(power_mode_data)) {
        if (cpu_dvfs_is(p,MT_CPU_DVFS_LITTLE)) 
            limit_freq = power_mode_data->cpu_freq_little;
        else
            limit_freq = power_mode_data->cpu_freq_big;

        idx = _search_available_freq_idx(p, limit_freq, CPUFREQ_RELATION_H);
        if (new_opp_idx < idx)
        {
            //pr_err("[power_mode_setting]: new_opp_idx=%d, limit_freq_idx=%d. \n", new_opp_idx, idx);
            new_opp_idx = idx;
        }
    }
    #endif
...
static int power_mode_setting(struct power_mode_info *power_mode_data) {
	...
	for_each_cpu_dvfs(i, p) {
		...
		if (cpu_dvfs_is(p,MT_CPU_DVFS_LITTLE)) {
			...
			cpufreq_cpu_put(policy);
            pr_err("[power_mode_setting]: cpu_freq_little=%d, little_turbo=%d. \n", power_mode_data->cpu_freq_little, power_mode_data->little_turbo);
		} else {
			...
			cpufreq_driver_target(policy, p->limited_max_freq, CPUFREQ_RELATION_H);
			cpufreq_cpu_put(policy);
            pr_err("[power_mode_setting]: cpu_freq_big=%d, big_turbo=%d. \n", power_mode_data->cpu_freq_big, power_mode_data->big_turbo);
		}

	}

    pr_err("[power_mode_setting]: little_num=%d, big_num=%d. \n", power_mode_data->little_num, power_mode_data->big_num);
