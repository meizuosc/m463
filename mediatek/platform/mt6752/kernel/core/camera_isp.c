...
#define LOG_WRN(format, args...)    xlog_printk(ANDROID_LOG_DEBUG   , "ISP", "[%s] WARNING: " format, __FUNCTION__, ##args)
#define LOG_ERR(format, args...)    xlog_printk(ANDROID_LOG_DEBUG  , "ISP", "[%s, line%04d] ERROR: " format, __FUNCTION__, __LINE__, ##args)
#define LOG_AST(format, args...)    xlog_printk(ANDROID_LOG_DEBUG , "ISP", "[%s, line%04d] ASSERT: " format, __FUNCTION__, __LINE__, ##args)
...
//            case ISP_RT_BUF_CTRL_EXCHANGE_ENQUE:
                ...
                if(copy_from_user(&rt_buf_info, (void*)rt_buf_ctrl.data_ptr, sizeof(ISP_RT_BUF_INFO_STRUCT)) == 0) {
					...
					if(i < ISP_RT_BUF_SIZE)  //lcz@meizu.com add,to avoid buffer overflow
    ...
    void*   DeviceId)
{
    ...
    if(IspInfo.DebugMask & ISP_DBG_INT) {
	if(ISP_IRQ_TYPE_INT_CAMSV < ISP_IRQ_USER_MAX)   //lcz@meizu.com add,to avoid buffer overflow
    ...
    void*   DeviceId)
{
    ...
    if(IspInfo.DebugMask & ISP_DBG_INT)
    {
	if(ISP_IRQ_TYPE_INT_CAMSV2 < ISP_IRQ_USER_MAX)   //lcz@meizu.com add,to avoid buffer overflow
...
//    LOG_DBG("- E.");
    ...
    if(IrqStatus[ISP_IRQ_TYPE_INT_P1_ST] & ISP_IRQ_P1_STATUS_SOF1_INT_ST)
    {
     	...
     	if (sof_cnt == 0) {
     		t_end = t_tmp;
     //		timespec_add_ns(&t_end, (3 * NSEC_PER_SEC));
     		t_end.tv_sec += 10;
     //		printk("=======start=======t_tmp:%ld======t_end:%ld=========\n", t_tmp.tv_sec, t_end.tv_sec);
     	}
     	sof_cnt++;
     	if (timespec_compare(&t_tmp, &t_end) >= 0) {
     		sof_cnt--;
     		fps = (10 * sof_cnt) / 10;
     //		printk("=======end=======t_tmp:%ld======t_end:%ld=========\n", t_tmp.tv_sec, t_end.tv_sec);
     		printk("===============P1_SOF1_INT_ST============\n");
     		printk("=====sof_cnt_per_10s:%d====fps:%d======\n", sof_cnt, fps);
     		sof_cnt = 0;
	    	}
        MUINT32 _dmaport = 0;
...
MBOOL ISP_RegCallback(ISP_CALLBACK_STRUCT* pCallback)
{
    ...
    //lcz@meizu.com add,for avoid buffer overflow
    if(pCallback->Type >= ISP_CALLBACK_AMOUNT) 
    {
        LOG_ERR("pCallback->Type(%d) must smaller than %d",pCallback->Type,ISP_CALLBACK_AMOUNT);
        return MFALSE;
    }
    //add end
...
{
    if(Type >= ISP_CALLBACK_AMOUNT)   //lcz@meizu.com modify,for avoid Type ==ISP_CALLBACK_AMOUNT overflow
    {
