	...
	*/
    ...
    case SHF_IPI_SEND: {
        ...
        if(copy_from_user((void*)&in_data, data, sizeof(ipi_data_t))) {
			err = -EFAULT;
            SHF_ERR("IOCTL: SHF_IPI_SEND copy failed!\n");
			break;	  
		}
        ...
        do {
            ...
            if ((times % 100 == 1))
                SHF_LOG("SHF_IPI_SEND: times=%d, size=%d, status=%d\n", times, in_data.size, status);
            err = status;
            mdelay(10);
        } while(DONE != status && times < 1000);
        mdelay(100);
        //SHF_LOG("SHF_IPI_SEND: delayed 100ms.\n");
    }   break;
    case SHF_IPI_POLL: {
        ...
        while(!ring_buffer_poll(&trigger_buffer, &temp_data)) {
            wait();
        }
        if(copy_to_user(data, &temp_data, sizeof(ipi_data_t))) {
			err = -EFAULT;
            SHF_ERR("IOCTL: SHF_IPI_POLL copy failed!\n");
			break;
		}
    }   break;
...
static int shf_driver_probe(struct platform_device *pdev)
{
    ...
    int st = md32_ipi_registration(IPI_SHF, shf_ipi_receive_handler, "shf_ipi_receive_handler");
    if (DONE != st) {
