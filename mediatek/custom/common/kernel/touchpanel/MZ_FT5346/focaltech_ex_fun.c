...
static u32 upgrade_state = 0;

//upgrade app
#ifdef MEIZU_M79

static unsigned char gisFW_APP_AUO_B[] =
{
#include "FT5346_V16_M79_GIS_AUO_B_20150106_app.i"
};

static unsigned char gisFW_APP_AUO_W[] =
{
#include "FT5346_V16_M79_GIS_AUO_W_20150106_app.i"
};

struct CtpFwInfo ctp_fw_app[] =
{
	/*gis + auo + white*/
	{0xcb, 0x31, gisFW_APP_AUO_W, sizeof(gisFW_APP_AUO_W)},
	/*gis + auo + black*/
	{0xcb, 0x32, gisFW_APP_AUO_B, sizeof(gisFW_APP_AUO_B)},
};

#else // MEIZU M71

static unsigned char aucFW_APP_AUO_W[] =
{
#include "FT5346_V1E_M71_OFILM_AUO_W_20150106_app.i"
};
static unsigned char aucFW_APP_AUO_B[] =
{
#include "FT5346_V1E_M71_OFILM_AUO_B_20150106_app.i"
};

static unsigned char aucFW_APP_GIS_W[] =
{
#include "FT5346_V05_M71_GIS_AUO_W_20150106_app.i"
};
static unsigned char aucFW_APP_GIS_B[] =
{
#include "FT5346_V05_M71_GIS_AUO_B_20150106_app.i"
};

static unsigned char aucFW_APP_SHARP_W[] =
{
#include "FT5346_V0A_M71_OFILM_SHARP_W_20141015_app.i"
};
static unsigned char aucFW_APP_SHARP_B[] =
{
#include "FT5346_V0A_M71_OFILM_SHARP_B_20141015_app.i"
};

struct CtpFwInfo ctp_fw_app[] =
{
	/*auo*/
	{0x51, 0x11, aucFW_APP_AUO_W, sizeof(aucFW_APP_AUO_W)},
	/*old fw, flash auo black*/
	{0x51, 0x1, aucFW_APP_AUO_B, sizeof(aucFW_APP_AUO_B)},
	/*old fw, flash sharp black*/
	{0x51, 0x2, aucFW_APP_SHARP_B, sizeof(aucFW_APP_SHARP_B)},
	/*auo*/
	{0x51, 0x12, aucFW_APP_AUO_B, sizeof(aucFW_APP_AUO_B)},
	/*sharp*/
	{0x51, 0x21, aucFW_APP_SHARP_W, sizeof(aucFW_APP_SHARP_W)},
	/*sharp*/
	{0x51, 0x22, aucFW_APP_SHARP_B, sizeof(aucFW_APP_SHARP_B)},
	/*gis + auo + black*/
	{0xcb, 0x14, aucFW_APP_GIS_B, sizeof(aucFW_APP_GIS_B)},
	/*gis + auo + white*/
	{0xcb, 0x13, aucFW_APP_GIS_W, sizeof(aucFW_APP_GIS_W)},
};
...
int fts_i2c_Write(struct i2c_client *client, u8 *writebuf, int writelen)
{
	int ret;
	int i = 0;
	//FTS_DBG("fts_i2c_Write  %d  %x  %x %x %x",writelen,writebuf,I2CDMABuf_va,I2CDMABuf_pa,client);
	...
	if (writelen > 0) {
		{
			...
			if((ret=i2c_master_send(g_focalclient, (unsigned char *)I2CDMABuf_pa, writelen))!=writelen)
				FTS_DBG( "###%s i2c write len=%x,buffaddr=%x\n", __func__,ret,I2CDMABuf_pa);
			//MSE_ERR("Sensor dma timing is %x!\r\n", this_client->timing);
			//return ret;
			g_focalclient->addr = g_focalclient->addr & I2C_MASK_FLAG &(~ I2C_DMA_FLAG);
	...
	if(readlen > 0){
		{
			...
			g_focalclient->addr = g_focalclient->addr & I2C_MASK_FLAG | I2C_DMA_FLAG;
			ret = i2c_master_recv(g_focalclient, (unsigned char *)I2CDMABuf_pa, readlen);
...
int focal_i2c_Write(unsigned char *writebuf, int writelen)
{
	...
	g_focalclient->addr = g_focalclient->addr & I2C_MASK_FLAG;
...
ft_fw_reflash_1:
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++)
	{
		...
		else
		{
			pr_err("[FTS] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",
			        reg_val[0], reg_val[1]);
			set_bit(2, &upgrade_state);
			continue;
		}
	}

	if (i >= FTS_UPGRADE_LOOP) {
		set_bit(7, &upgrade_state);
		return -EIO;
	}
...
ft_fw_reflash_4:
	...
	if (reg_val[0] != bt_ecc)
	{
		...
		if (reflash_flag) {
			...
			set_bit(8, &upgrade_state);
			return -EIO;
		} else {
			reflash_flag++;
			pr_err("[FTS] check ecc error, reflash again\n");
			set_bit(5, &upgrade_state);
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	if(i >= 5) {
		pr_err("!!! firmware not match vendor_id %02x lcm_id %02x, exit\n",
				ctp_info.vendor_id, ctp_info.lcm_id);
#ifdef MEIZU_M79
		// add default option, to upgrade fw if lcm_id is not exist
		fw_data = gisFW_APP_AUO_B;
		fw_length = sizeof(gisFW_APP_AUO_B);
#else
		goto fw_exit;
#endif
	}
                                  ...
                                  const char *buf, size_t count)
{
	...
	mt_eint_mask(CUST_EINT_CTP_NUM);
	...
	*/
	...
	mt_eint_unmask(CUST_EINT_CTP_NUM);
	mutex_unlock(&g_device_mutex);
...
#ifdef MEIZU_M79
	count += sprintf(tmp + count, "79");
#else
	count += sprintf(tmp + count, "71");
#endif

#ifdef MEIZU_M79
	if((cur_tp_info.lcm_id & 0xf0) == 0x30)
		count += sprintf(tmp + count, "GIS");
	else // 0x10
...
static ssize_t ft5x46_upgrade_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
...
/*get project code*/
static DEVICE_ATTR(ftsgetprojectcode, S_IRUGO|S_IWUSR, ftxxxx_ftsgetprojectcode_show,
                   ftxxxx_ftsgetprojectcode_store);
...
static DEVICE_ATTR(upgrade, S_IRUGO|S_IWUSR, ft5x46_upgrade_show, NULL);
#endif
...
int fts_dma_buffer_init(void)
{
	...
	FTS_DBG("[FTS] fts_dma_buffer_init I2CDMABuf_pa=%x,val=%x val2=%x\n",&I2CDMABuf_pa,I2CDMABuf_pa,(unsigned char *)I2CDMABuf_pa);
	return 0;
}
