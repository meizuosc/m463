...
extern void vibrator_on_time(int value);
...
#ifdef MZ_HALL_MODE
#ifdef MEIZU_M79
    u8 cfg_info_hall_mode1[] = CTP_CFG_HALL_MODE1;
    u8 cfg_info_hall_mode2[] = CTP_CFG_HALL_MODE2;
#else // M71
...
#ifdef MEIZU_M79
    if(sensor_id == 0 || sensor_id == 2) { // TRU
	memcpy(&config_hall_mode[GTP_ADDR_LENGTH], cfg_info_hall_mode1,
			CFG_GROUP_LEN(cfg_info_hall_mode1));
    } else { // OFM
	memcpy(&config_hall_mode[GTP_ADDR_LENGTH], cfg_info_hall_mode2,
			CFG_GROUP_LEN(cfg_info_hall_mode2));
    }
#else // M71
...
//#ifdef MT6572
    ...
    {
        mt_eint_registration(CUST_EINT_CTP_NUM, EINTF_TRIGGER_RISING, tpd_eint_interrupt_handler, 1);
    }
    else
    {
        mt_eint_registration(CUST_EINT_CTP_NUM, EINTF_TRIGGER_FALLING, tpd_eint_interrupt_handler, 1);
    }
    
#if 0
    mt65xx_eint_set_sens(CUST_EINT_CTP_NUM, CUST_EINT_TOUCH_PANEL_SENSITIVE);
    mt65xx_eint_set_hw_debounce(CUST_EINT_CTP_NUM, CUST_EINT_TOUCH_PANEL_DEBOUNCE_CN);

    if (!int_type)
    {
        mt65xx_eint_registration(CUST_EINT_CTP_NUM, CUST_EINT_TOUCH_PANEL_DEBOUNCE_EN, CUST_EINT_POLARITY_HIGH, tpd_eint_interrupt_handler, 1);
    }
    else
    {
        mt65xx_eint_registration(CUST_EINT_CTP_NUM, CUST_EINT_TOUCH_PANEL_DEBOUNCE_EN, CUST_EINT_POLARITY_LOW, tpd_eint_interrupt_handler, 1);
    }
#endif
...
static int touch_event_handler(void *unused)
{
	...
	if ((finger & 0x40) != 0)
	{
        ...
        if (key_value || pre_key)
        {
		    ...
		    if(key_value) vibrator_on_time(80);
                }
        ...
        else
        {
			...
			tpd_up(0, 0, 0);
#endif
                GTP_DEBUG("Touch Release: %d, finger_status: %04x\n",
