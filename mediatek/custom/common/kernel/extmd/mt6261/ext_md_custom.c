...
int cm_do_md_power_on(int bootmode)
{
    ...
    if(mt_get_gpio_out(GPIO_EXT_MD_PWR_KEY)==1)
    {
        EMD_MSG_INF("chr","cm_do_md_power_on:reset dl power \n");
        // Release download key to let md can enter normal boot
        mt_set_gpio_dir(GPIO_EXT_MD_DL_KEY, 1);
        mt_set_gpio_out(GPIO_EXT_MD_DL_KEY, 0);
        // Press power key            
    ...
    mt_set_gpio_dir(GPIO_EXT_MD_PWR_KEY, 1);
    mt_set_gpio_out(GPIO_EXT_MD_PWR_KEY, 1);
    EMD_MSG_INF("chr","cm_do_md_power_on:set GPIO_EXT_MD_PWR_KEY(%d)\n",mt_get_gpio_out(GPIO_EXT_MD_PWR_KEY));
    if(bootmode)
    {
    ...
    msleep(270);
    EMD_MSG_INF("chr","cm_do_md_power_on: GPIO_EXT_MD_EXP(%d),GPIO_EXT_MD_WD(%d),GPIO_EXT_MD_RST(%d)\n", \
    mt_get_gpio_in(GPIO_EXT_MD_EXP),mt_get_gpio_in(GPIO_EXT_MD_WD),mt_get_gpio_out(GPIO_EXT_MD_RST));
...
void cm_gpio_setup(void)
{
    ...
    mt_set_gpio_dir(GPIO_EXT_MD_PWR_KEY, 0);// Using input floating
