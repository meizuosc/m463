...
static struct i2c_board_info __initdata i2c_lsm303c={ I2C_BOARD_INFO("lsm303c-acc", 0x3B>>1)};
...
static int lsm303c_CheckDeviceID(struct i2c_client *client)
{
	...
	{
        GSE_ERR("Check ID error: id = 0x%x != LSM303C_FIXED_DEVID[]\n", databuf[0], LSM303C_FIXED_DEVID);
...
static int lsm303c_ReadCalibrationEx(struct i2c_client *client, int act[LSM303C_AXES_NUM], int raw[LSM303C_AXES_NUM])
{
	...
	raw[LSM303C_AXIS_X] = obj->offset[LSM303C_AXIS_X]*mul + obj->cali_sw[LSM303C_AXIS_X];
	raw[LSM303C_AXIS_Y] = obj->offset[LSM303C_AXIS_Y]*mul + obj->cali_sw[LSM303C_AXIS_Y];
	raw[LSM303C_AXIS_Z] = obj->offset[LSM303C_AXIS_Z]*mul + obj->cali_sw[LSM303C_AXIS_Z];

	act[obj->cvt.map[LSM303C_AXIS_X]] = obj->cvt.sign[LSM303C_AXIS_X]*raw[LSM303C_AXIS_X];
	act[obj->cvt.map[LSM303C_AXIS_Y]] = obj->cvt.sign[LSM303C_AXIS_Y]*raw[LSM303C_AXIS_Y];
	act[obj->cvt.map[LSM303C_AXIS_Z]] = obj->cvt.sign[LSM303C_AXIS_Z]*raw[LSM303C_AXIS_Z];
...
static int lsm303c_ReadCalibration(struct i2c_client *client, int dat[LSM303C_AXES_NUM])
{
   ...
   dat[obj->cvt.map[LSM303C_AXIS_X]] = obj->cvt.sign[LSM303C_AXIS_X]*(obj->offset[LSM303C_AXIS_X]*mul + obj->cali_sw[LSM303C_AXIS_X]);
   dat[obj->cvt.map[LSM303C_AXIS_Y]] = obj->cvt.sign[LSM303C_AXIS_Y]*(obj->offset[LSM303C_AXIS_Y]*mul + obj->cali_sw[LSM303C_AXIS_Y]);
   dat[obj->cvt.map[LSM303C_AXIS_Z]] = obj->cvt.sign[LSM303C_AXIS_Z]*(obj->offset[LSM303C_AXIS_Z]*mul + obj->cali_sw[LSM303C_AXIS_Z]);
...
#define LSM303C_AXIS_Z          2
...
#ifdef SW_CALIBRATION
	obj->cali_sw[LSM303C_AXIS_X] = obj->cvt.sign[LSM303C_AXIS_X]*(cali[obj->cvt.map[LSM303C_AXIS_X]]);
	obj->cali_sw[LSM303C_AXIS_Y] = obj->cvt.sign[LSM303C_AXIS_Y]*(cali[obj->cvt.map[LSM303C_AXIS_Y]]);
	obj->cali_sw[LSM303C_AXIS_Z] = obj->cvt.sign[LSM303C_AXIS_Z]*(cali[obj->cvt.map[LSM303C_AXIS_Z]]);
	...
	GSE_LOG("NEWOFF: (%+3d %+3d %+3d): (%+3d %+3d %+3d) / (%+3d %+3d %+3d)\n",
		...
		obj->cali_sw[LSM303C_AXIS_X], obj->cali_sw[LSM303C_AXIS_Y], obj->cali_sw[LSM303C_AXIS_Z]);
#else
	obj->offset[LSM303C_AXIS_X] = (s8)(obj->cvt.sign[LSM303C_AXIS_X]*(cali[obj->cvt.map[LSM303C_AXIS_X]])/(divisor));
	obj->offset[LSM303C_AXIS_Y] = (s8)(obj->cvt.sign[LSM303C_AXIS_Y]*(cali[obj->cvt.map[LSM303C_AXIS_Y]])/(divisor));
	obj->offset[LSM303C_AXIS_Z] = (s8)(obj->cvt.sign[LSM303C_AXIS_Z]*(cali[obj->cvt.map[LSM303C_AXIS_Z]])/(divisor));

	/*convert software calibration using standard calibration*/
	obj->cali_sw[LSM303C_AXIS_X] = obj->cvt.sign[LSM303C_AXIS_X]*(cali[obj->cvt.map[LSM303C_AXIS_X]])%(divisor);
	obj->cali_sw[LSM303C_AXIS_Y] = obj->cvt.sign[LSM303C_AXIS_Y]*(cali[obj->cvt.map[LSM303C_AXIS_Y]])%(divisor);
	obj->cali_sw[LSM303C_AXIS_Z] = obj->cvt.sign[LSM303C_AXIS_Z]*(cali[obj->cvt.map[LSM303C_AXIS_Z]])%(divisor);
...
static int lsm303c_init_client(struct i2c_client *client, int reset_cali)
{
	...
	// 4 RANGE  ok
	res = lsm303c_SetDataFormat(client, LSM303C_ACC_FS_2G);
	if(res != LSM303C_SUCCESS) //0x2C->BW=100Hz
	{
...
static int lsm303c_ReadData(struct i2c_client *client, s32 data[LSM303C_AXES_NUM])
{
	...
	else
	{
		...
		{
			GSE_LOG("transfer before [%08X %08X %08X] => [%08X %08X %08X]\n", buf[LSM303C_AXIS_X*2],buf[LSM303C_AXIS_X*2+1],
						buf[LSM303C_AXIS_Y*2], buf[LSM303C_AXIS_Y*2+1], buf[LSM303C_AXIS_Z*2], buf[LSM303C_AXIS_Z*2+1]);
		}
	...
	/******
		...
		data[LSM303C_AXIS_Z] = (s16)((buf[LSM303C_AXIS_Z*2]) |
		         ...
		         ***/
		/***
		data[LSM303C_AXIS_X] += priv->cali_sw[LSM303C_AXIS_X];
		data[LSM303C_AXIS_Y] += priv->cali_sw[LSM303C_AXIS_Y];
		data[LSM303C_AXIS_Z] += priv->cali_sw[LSM303C_AXIS_Z];
		***/
		if(atomic_read(&priv->trace) & ADX_TRC_RAWDATA)
		{
			...
			if(atomic_read(&priv->fir_en) && !atomic_read(&priv->suspend))
			{
				...
				if(priv->fir.num < firlen)
				{
					...
					if(atomic_read(&priv->trace) & ADX_TRC_FILTER)
					{
						GSE_LOG("add [%2d] [%5d %5d %5d] => [%5d %5d %5d]\n", priv->fir.num,
							priv->fir.raw[priv->fir.num][LSM303C_AXIS_X], priv->fir.raw[priv->fir.num][LSM303C_AXIS_Y], priv->fir.raw[priv->fir.num][LSM303C_AXIS_Z],
							priv->fir.sum[LSM303C_AXIS_X], priv->fir.sum[LSM303C_AXIS_Y], priv->fir.sum[LSM303C_AXIS_Z]);
					}
					priv->fir.num++;
					priv->fir.idx++;
				}
				else
				{
					...
					if(atomic_read(&priv->trace) & ADX_TRC_FILTER)
					{
						...
						priv->fir.raw[idx][LSM303C_AXIS_X], priv->fir.raw[idx][LSM303C_AXIS_Y], priv->fir.raw[idx][LSM303C_AXIS_Z],
						priv->fir.sum[LSM303C_AXIS_X], priv->fir.sum[LSM303C_AXIS_Y], priv->fir.sum[LSM303C_AXIS_Z],
						data[LSM303C_AXIS_X], data[LSM303C_AXIS_Y], data[LSM303C_AXIS_Z]);
					}
				}
...
static int lsm303c_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	else
	{
        ...
        // GSE_LOG("limingggGSE %d %d %d", acc[LSM303C_AXIS_X], acc[LSM303C_AXIS_Y], acc[LSM303C_AXIS_Z]);

		sprintf(buf, "%04x %04x %04x", acc[LSM303C_AXIS_X], acc[LSM303C_AXIS_Y], acc[LSM303C_AXIS_Z]);
...
static int lsm303c_InitSelfTest(struct i2c_client *client)
{
	...
	res = hwmsen_read_byte(client, LSM303C_REG_DATA_FORMAT, &data);
	if(res != LSM303C_SUCCESS)
	{
		return res;
	}
	//res = lsm303c_SetDataFormat(client, LSM303C_SELF_TEST|data);
...
/**
    ...
    u8 format;
    int res;
    if((res = hwmsen_read_byte(client, LSM303C_REG_DATA_FORMAT, &format)))
        return res;
    if(format & LSM303C_FULL_RES)
        ptr = &self[0];
    else if ((format & LSM303C_RANGE_4G))
        ptr = &self[1];
    else if ((format & LSM303C_RANGE_8G))
        ptr = &self[2];
    else if ((format & LSM303C_RANGE_16G))
        ptr = &self[3];
    ...
    if (((nxt[LSM303C_AXIS_X] - prv[LSM303C_AXIS_X]) > (*ptr)[LSM303C_AXIS_X].max) ||
        ((nxt[LSM303C_AXIS_X] - prv[LSM303C_AXIS_X]) < (*ptr)[LSM303C_AXIS_X].min)) {
        GSE_ERR("X is over range\n");
        res = -EINVAL;
    }
    if (((nxt[LSM303C_AXIS_Y] - prv[LSM303C_AXIS_Y]) > (*ptr)[LSM303C_AXIS_Y].max) ||
        ((nxt[LSM303C_AXIS_Y] - prv[LSM303C_AXIS_Y]) < (*ptr)[LSM303C_AXIS_Y].min)) {
        GSE_ERR("Y is over range\n");
        res = -EINVAL;
    }
    if (((nxt[LSM303C_AXIS_Z] - prv[LSM303C_AXIS_Z]) > (*ptr)[LSM303C_AXIS_Z].max) ||
        ((nxt[LSM303C_AXIS_Z] - prv[LSM303C_AXIS_Z]) < (*ptr)[LSM303C_AXIS_Z].min)) {
...
{   /*write anything to this register will trigger the process*/
	struct item{
	s16 raw[LSM303C_AXES_NUM];
	};

	struct i2c_client *client = lsm303c_i2c_client;
	int idx, res, num;
	struct item *prv = NULL, *nxt = NULL;
	s32 avg_prv[LSM303C_AXES_NUM] = {0, 0, 0};
	s32 avg_nxt[LSM303C_AXES_NUM] = {0, 0, 0};
	...
	GSE_LOG("NORMAL:\n");
	lsm303c_SetPowerMode(client,true);
	for(idx = 0; idx < num; idx++)
	{
		if((res = lsm303c_ReadData(client, prv[idx].raw)))
		{
			GSE_ERR("read data fail: %d\n", res);
			goto exit;
		}

		avg_prv[LSM303C_AXIS_X] += prv[idx].raw[LSM303C_AXIS_X];
		avg_prv[LSM303C_AXIS_Y] += prv[idx].raw[LSM303C_AXIS_Y];
		avg_prv[LSM303C_AXIS_Z] += prv[idx].raw[LSM303C_AXIS_Z];
		GSE_LOG("[%5d %5d %5d]\n", prv[idx].raw[LSM303C_AXIS_X], prv[idx].raw[LSM303C_AXIS_Y], prv[idx].raw[LSM303C_AXIS_Z]);
	}

	avg_prv[LSM303C_AXIS_X] /= num;
	avg_prv[LSM303C_AXIS_Y] /= num;
	avg_prv[LSM303C_AXIS_Z] /= num;

	//initial setting for self test
	lsm303c_InitSelfTest(client);
	GSE_LOG("SELFTEST:\n");
	for(idx = 0; idx < num; idx++)
	{
		if((res = lsm303c_ReadData(client, nxt[idx].raw)))
		{
			GSE_ERR("read data fail: %d\n", res);
			goto exit;
		}
		avg_nxt[LSM303C_AXIS_X] += nxt[idx].raw[LSM303C_AXIS_X];
		avg_nxt[LSM303C_AXIS_Y] += nxt[idx].raw[LSM303C_AXIS_Y];
		avg_nxt[LSM303C_AXIS_Z] += nxt[idx].raw[LSM303C_AXIS_Z];
		GSE_LOG("[%5d %5d %5d]\n", nxt[idx].raw[LSM303C_AXIS_X], nxt[idx].raw[LSM303C_AXIS_Y], nxt[idx].raw[LSM303C_AXIS_Z]);
	}

	avg_nxt[LSM303C_AXIS_X] /= num;
	avg_nxt[LSM303C_AXIS_Y] /= num;
	avg_nxt[LSM303C_AXIS_Z] /= num;

	GSE_LOG("X: %5d - %5d = %5d \n", avg_nxt[LSM303C_AXIS_X], avg_prv[LSM303C_AXIS_X], avg_nxt[LSM303C_AXIS_X] - avg_prv[LSM303C_AXIS_X]);
	GSE_LOG("Y: %5d - %5d = %5d \n", avg_nxt[LSM303C_AXIS_Y], avg_prv[LSM303C_AXIS_Y], avg_nxt[LSM303C_AXIS_Y] - avg_prv[LSM303C_AXIS_Y]);
	GSE_LOG("Z: %5d - %5d = %5d \n", avg_nxt[LSM303C_AXIS_Z], avg_prv[LSM303C_AXIS_Z], avg_nxt[LSM303C_AXIS_Z] - avg_prv[LSM303C_AXIS_Z]);

	if(!lsm303c_JudgeTestResult(client, avg_prv, avg_nxt))
	{
...
/*----------------------------------------------------------------------------*/
//static int lsm303c_ioctl(struct inode *inode, struct file *file, unsigned int cmd,
//       unsigned long arg)
