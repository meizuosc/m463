...
#define AF_DRVNAME "AD5823AF"
#define I2C_SLAVE_ADDRESS        0x18
#define I2C_REGISTER_ID            0x6C
#define PLATFORM_DRIVER_NAME "lens_actuator_ad5823af"
#define AF_DRIVER_CLASS_NAME "actuatordrv_ad5823af"
...
static int s4AF_ReadReg(unsigned short * a_pu2Result)
{
	...
	LOG_INF("s4AF_ReadReg VCM MSB: %d \n", pBuff[1]);
	if (i4RetValue < 0) 
	{
		LOG_INF("I2C send failed!! \n");
		return -1;
	...
	LOG_INF("s4AF_ReadReg VCM LSB: %d \n",pBuff[0]);
	if (i4RetValue < 0) 
	{
		LOG_INF("I2C send failed!! \n");
		return -1;
...
static int s4AF_WriteReg(u16 a_u2Data)
{
	...
	//0x04[2] RING_CTRL   0: direct mode  1: enable ringing control
	//0x04[1:0] VCM MSB data
	//0x05[7:0] VCM LSB data

#ifdef USE_ISRC_MODE
	char VCMMSB[2] = {(char)(0x04) , (char)((a_u2Data >> 8) & 0x03 )+ 0x04};
...
inline static int getAFInfo(__user stAD5823AF_MotorInfo * pstMotorInfo)
{
...
#ifdef LensdrvCM3
inline static int getAFMETA(__user stAD5823AF_MotorMETAInfo * pstMotorMETAInfo)
{
    stAD5823AF_MotorMETAInfo stMotorMETAInfo;
    ...
    if(copy_to_user(pstMotorMETAInfo , &stMotorMETAInfo , sizeof(stAD5823AF_MotorMETAInfo)))
    {
        LOG_INF("copy to user failed when getting motor information \n");
    }
...
unsigned long a_u4Param)
{
    ...
    switch(a_u4Command)
    {
        case AD5823AFIOC_G_MOTORINFO :
            i4RetValue = getAFInfo((__user stAD5823AF_MotorInfo *)(a_u4Param));
        break;
        #ifdef LensdrvCM3
        case AD5823AFIOC_G_MOTORMETAINFO :
            i4RetValue = getAFMETA((__user stAD5823AF_MotorMETAInfo *)(a_u4Param));
        break;
        #endif
