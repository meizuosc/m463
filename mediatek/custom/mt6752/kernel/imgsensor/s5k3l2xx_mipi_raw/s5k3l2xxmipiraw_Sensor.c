...
//		read_otp_byte(addr+9,&pOtp->production_day,write_id);
		...
		read_otp_byte(addr+34,&pOtp->otp_af->underspin_flag,write_id); 
			...
			read_otp_byte(addr+37,&low,write_id); 
			read_otp_byte(addr+38,&high,write_id);
			pOtp->otp_af->af_infinity = high<<8|low;
			af_inf_pos = pOtp->otp_af->af_infinity;
			
			read_otp_byte(addr+39,&low,write_id); 
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		if(module_id == PRIMAX_MODULE_ID){
			...
			if(program_id == 0x4D){
                	LOG_INF("i2c write id: 0x%x, sensor id: 0x%x,pimax module id:%x\n", imgsensor.i2c_write_id,tmp,module_id);
    ...
    if (*sensor_id != imgsensor_info.sensor_id || module_id != PRIMAX_MODULE_ID) {
